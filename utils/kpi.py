...
def show_kpi_panel(df):
    ...
    total_be = df.get("Kümüle BE Bakiye", 0).sum() if "Kümüle BE Bakiye" in df.columns else 0
    ...
    usage_pct = (total_actual / total_budget * 100) if total_budget else 0
    be_ratio = (total_be / total_actual * 100) if total_actual else 0
    karsilik_ratio = (total_karsilik / total_actual * 100) if total_actual else 0
    ...
    with col2:
        ...
        st.metric("📊 Bütçe Farkı", f"{variance:,.0f} ₺", delta=f"{variance_pct:.2f} %", delta_color=delta_color)
