...
def generate_insights(df):
    ...
    if "Kümüle Bütçe" in df.columns and "Kümüle Fiili" in df.columns:
        ...
        if not active.empty:
            min_row = active.groupby("Masraf Yeri Adı")["Kümüle Fiili"].sum().sort_values().head(1)
        ...
        if "Masraf Yeri Adı" in df.columns:
            ...
            kullanim_df["Kullanım Oranı"] = kullanim_df["Kümüle Fiili"] / kullanim_df["Kümüle Bütçe"]
            if not kullanim_df.empty:
                max_row = kullanim_df.sort_values("Kullanım Oranı", ascending=False).head(1)
                oran = max_row["Kullanım Oranı"].values[0] * 100
                ad = max_row["Masraf Yeri Adı"].values[0]
                insights.append(f"🔥 En yüksek bütçe kullanım oranı: **{ad}** (%{oran:.1f})")

    if "Masraf Çeşidi Grubu 1" in df.columns and "Kümüle Fiili" in df.columns:
        top_grup = df.groupby("Masraf Çeşidi Grubu 1")["Kümüle Fiili"].sum().sort_values(ascending=False).head(1)
